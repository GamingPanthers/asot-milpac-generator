...
/**
 * Sync Panther's corps from milpacs (authoritative) to members collection
 */
async function syncPantherCorps() {
  try {
    ...
    console.log('  Current Corps:', milpac.corps);
    console.log('  Rank:', milpac.rankName);

    // Normalize corps - convert full names to abbreviated form
    let corpsNormalized = milpac.corps;
    if (typeof corpsNormalized === 'string') {
      // Convert "Army Aviation Corp" -> "Aviation"
      corpsNormalized = corpsNormalized
        .replace('Army ', '')
        .replace(' Corp', '')
        .trim();
    }
